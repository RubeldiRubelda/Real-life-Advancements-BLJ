...
router.use("/save", (req, res) => {
  ...
  res.json({"Okay": true, "Users": buffer, "Passwords": "HIDDEN"})
//   res.json({"Okay": true, "Users": buffer, "Passwords": buffer2})
...
router.use("/load", (req, res) => {
  ...
  res.json({"Okay": true, "Users": JSON.stringify(user), "Passwords": "HIDDEN"})
//   res.json({"Okay": true, "Users": JSON.stringify(user), "Passwords": JSON.stringify(passwords)})
...
router.use("/add/:user/:pw", (req, res) => {
    ...
    if (userid == -1) {
        ...
        res.json({"Okay": false, "Error": "User already exists.", "UserID": userid})
...
router.use("/del/:user/:pw", (req, res) => {
    let userid = user.indexOf(req.params.user)
    if (userid == -1) {
        res.json({"Okay": false, "Error": "User or Password dose not exist do not match."})
        ...
        if (passwd == req.params.pw) {
            ...
            res.json({"Okay": true, "UserID": userid, "Task": "User was now deleted."})
...
router.use("/check/:user/:pw", (req, res) => {
    ...
    if (userid == -1) {
        res.json({"Okay": false, "Error": "User or Password dose not exist do not match."})
        console.log(logprefix + "User: \"" + req.params.user + "\" tryed to check in but he is not registerd.")
    } else {
        let passwd = passwords[userid]
        if (passwd == req.params.pw) {
            res.json({"Okay": true, "UserID": userid})
            console.log(logprefix + "User: \"" + req.params.user + "\" checked in with id: \"" + userid + "\" succesfully.")
        } else {
            res.json({"Okay": false, "Error": "User or Password dose not exist do not match."})
            console.log(logprefix + "User: \"" + req.params.user + "\" with id: \"" + userid + "\" tryed to check in but the password was wrong.")
        }
    }
})
